...
import { Text, View, SafeAreaView, ScrollView, TouchableOpacity, Modal, Pressable, Platform, StatusBar, FlatList, Switch, Alert } from "react-native";
...
export default function App() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // const restorePref = await AsyncStorage.getItem(RESTORE_KEY);
        // const savedMeta = await AsyncStorage.getItem(TABS_META_KEY);
  ...
  const addTab = () => {
    // const usedNumbers = new Set(tabs.map((t) => {
  ...
  const reportUsage = (tabId, mode, isUsed) => {
    if (!tabUsageRef.current[tabId]) tabUsageRef.current[tabId] = { calc: false, finance: false, conv: false };
  ...
  const handleRequestNewTab = (expr, mode, category, data = null) => {
    ...
    if (!targetMode) {
      if (expr?.match(/^(EMI:|Stock Avg:|Reg Int:|Simple Int:|Compound Int:|SIP:|GST:)/)) targetMode = "finance";
      else if (expr?.match(/^(Age:|BMI:|Date Diff:|Time Zone:)| to /)) targetMode = "conv";
      else targetMode = "calc";
    }

    const usage = tabUsageRef.current[activeId] || {};
    const isTabUsed = usage.calc || usage.finance || usage.conv;
    ...
    setShowHist(false);
    setPendingLoad({ targetTabId, expression: expr, category: targetCategory, data });    
  };
  ...
  const closeTab = (id) => {
    ...
    AsyncStorage.removeItem(`@tab_data_${id}`).catch(() => {}); 
  };
    ...
    <SafeAreaView style={{ 
      ...
      <View style={{ flexDirection: "row", height: 50, backgroundColor: colors.nav, alignItems: "center", paddingHorizontal: 10 }}>
        <ScrollView 
          ...
          ref={tabsListRef}
          onContentSizeChange={() => tabsListRef.current?.scrollToEnd({ animated: true })}
        >
          {tabs.map((tab) => (
            <TouchableOpacity
              key={tab.id}
