...
const CATEGORIES = ["EMI", "Interest", "FD", "RD", "Stocks", "SIP", "GST"];
...
export default function Finance(props) {
  ...
  // --- FD STATE ---
  const [fdConfig, setFdConfig] = useState({ amount: "", rate: "", period: "", periodUnit: "Years", freq: "Quarterly" });
  const [fdResult, setFdResult] = useState(null);

  // --- RD STATE ---
  const [rdConfig, setRdConfig] = useState({ amount: "", rate: "", period: "", periodUnit: "Years", freq: "Quarterly" });
    ...
    const loadData = async () => {
        try {
            ...
            if (saved) {
                ...
                if (data.fd) setFdConfig(data.fd);
                if (data.rd) setRdConfig(data.rd);
  ...
  }, [category, emiConfig, fdConfig, rdConfig, stockConfig, legacyConfig, regConfig, sipConfig, gstConfig, intType, is30DayMode, isGstInclusive, tabId, dataLoaded]);

  useEffect(() => {
    if (!dataLoaded) return;
    switch (category) {
        case "EMI": calculateEMI(); break;
        case "Interest": intType === "Regular" ? calculateRegularInterest() : calculateInterestLegacy(); break;
        case "FD": calculateFD(); break;
        case "RD": calculateRD(); break;
  ...
  useEffect(() => {
    ...
    const isFdUsed = fdConfig.amount || fdConfig.rate || fdConfig.period;
    const isRdUsed = rdConfig.amount || rdConfig.rate || rdConfig.period;
    ...
    const isRegUsed = regConfig.principal || (regConfig.rate !== "2"); 
    const isSipUsed = sipConfig.investment || sipConfig.rate || sipConfig.years;
    const isGstUsed = gstConfig.amount;
    
    const isUsed = isEmiUsed || isFdUsed || isRdUsed || isStockUsed || isLegacyUsed || isRegUsed || isSipUsed || isGstUsed;
    if (reportUsage) reportUsage(tabId, 'finance', !!isUsed);
  }, [emiConfig, fdConfig, rdConfig, stockConfig, legacyConfig, regConfig, sipConfig, gstConfig, reportUsage, tabId]);
  ...
  const loadFromExpression = (expr) => {
    ...
    if (expr.startsWith("EMI:")) {
       ...
       setEmiConfig({ amount: p, rate: r, tenure: n, tenureUnit: unit });
       calculateEMI({ amount: p, rate: r, tenure: n, tenureUnit: unit });
    }
    else if (expr.startsWith("FD:")) {
        setCategory("FD");
        const parts = expr.replace("FD: ", "").split(" @ ");
        if (parts.length < 2) return;
        const p = parts[0];
        const rest = parts[1].split("% for ");
        const r = rest[0];
        const n = rest[1].replace("y", "").replace("m", "").replace("d", "");
        let unit = "Years";
        if (rest[1].includes("m")) unit = "Months";
        if (rest[1].includes("d")) unit = "Days";
        setFdConfig({ ...fdConfig, amount: p, rate: r, period: n, periodUnit: unit });
        calculateFD({ ...fdConfig, amount: p, rate: r, period: n, periodUnit: unit });
    }
    else if (expr.startsWith("RD:")) {
        setCategory("RD");
        const parts = expr.replace("RD: ", "").split(" @ ");
        if (parts.length < 2) return;
        const p = parts[0];
        const rest = parts[1].split("% for ");
        const r = rest[0];
        const n = rest[1].replace("y", "").replace("m", "");
        const unit = rest[1].includes("m") ? "Months" : "Years";
        setRdConfig({ ...rdConfig, amount: p, rate: r, period: n, periodUnit: unit });
        calculateRD({ ...rdConfig, amount: p, rate: r, period: n, periodUnit: unit });
    }
       ...
       if(left.length === 2 && right.length === 2) {
         setStockConfig({ q1: left[0], p1: left[1], q2: right[0], p2: right[1] });
         calculateStockAvg({ q1: left[0], p1: left[1], q2: right[0], p2: right[1] });
       }
    }
        ...
        if (unitAndDates.length >= 2) {
            ...
            if (dates.length >= 2) { newConf.start = dates[0]; newConf.end = dates[1]; }
        }
        let loadedMode = true; 
        ...
        setSipConfig({ investment: inv, rate: r, years: y });
        calculateSIP({ investment: inv, rate: r, years: y });
    }
        ...
        setGstConfig({ amount: amt, rate: r });
        setIsGstInclusive(typeStr === "Inclusive");
        calculateGST({ amount: amt, rate: r }, typeStr === "Inclusive");
    }
  };

  useEffect(() => { 
    if (pendingLoad?.targetTabId === tabId) {
      if (pendingLoad.mode !== 'finance') return;
      if (pendingLoad.category) setCategory(pendingLoad.category);
      loadFromExpression(pendingLoad.expression); onConsumeLoad();
  } }, [pendingLoad, tabId]);
  ...
  const isToday = (dateObj) => {
      const t = new Date();
      return dateObj.getDate() === t.getDate() && dateObj.getMonth() === t.getMonth() && dateObj.getFullYear() === t.getFullYear();
  };
  ...
  const calculateEMI = (overrideConfig) => {
    ...
    const res = {
      emi: emi.toFixed(2), total: totalPayment.toFixed(2), interest: totalInterest.toFixed(2),
      principal: P, principalPct: totalPayment > 0 ? P / totalPayment : 0, interestPct: totalPayment > 0 ? totalInterest / totalPayment : 0
    };
    setEmiResult(res); Keyboard.dismiss();
  ...
  const calculateFD = (overrideConfig) => {
    const data = overrideConfig || fdConfig;
    const { amount, rate, period, periodUnit, freq } = data;
    if (!amount || !rate || !period) return;
    const P = parseFloat(amount);
    const R = parseFloat(rate);
    const timeVal = parseFloat(period);
    
    let timeInYears = 0;
    if (periodUnit === 'Years') timeInYears = timeVal;
    else if (periodUnit === 'Months') timeInYears = timeVal / 12;
    else if (periodUnit === 'Days') timeInYears = timeVal / 365;
    ...
    const amountTotal = P * Math.pow(1 + (R / 100) / n, n * timeInYears);
    const interestTotal = amountTotal - P;

    const res = {
        total: amountTotal.toFixed(2),
        interest: interestTotal.toFixed(2),
        principal: P,
        interestPct: amountTotal > 0 ? interestTotal / amountTotal : 0
    };
    setFdResult(res); Keyboard.dismiss();

    if (!overrideConfig && onSave) {
        const uChar = periodUnit === 'Years' ? 'y' : (periodUnit === 'Months' ? 'm' : 'd');
        onSave(`FD: ${P} @ ${rate}% for ${period}${uChar}`, `Total: ${res.total}, Int: ${res.interest}`, "finance", "FD");
    }
  };

  const calculateRD = (overrideConfig) => {
      const data = overrideConfig || rdConfig;
      const { amount, rate, period, periodUnit, freq } = data;
      if (!amount || !rate || !period) return;
      const P = parseFloat(amount);
      const R = parseFloat(rate);
      ...
      let months = 0;
      if (periodUnit === 'Years') months = timeVal * 12;
      else months = timeVal;
      ...
      // RD Calculation via Summation of Compound Interest for each installment
      // This is generally more accurate for standard bank RDs than simplified formulas
      let totalMaturity = 0;
      const r_dec = R / 100;
      
      for (let i = 0; i < months; i++) {
          // Each installment stays invested for (TotalMonths - i) months
          const monthsRemaining = months - i;
          const yearsRemaining = monthsRemaining / 12.0;
          const installmentVal = P * Math.pow(1 + r_dec / n, n * yearsRemaining);
          totalMaturity += installmentVal;
      }
      ...
      const res = {
          total: totalMaturity.toFixed(2),
          invested: invested.toFixed(2),
          interest: interest.toFixed(2),
          interestPct: totalMaturity > 0 ? interest / totalMaturity : 0
      };
      setRdResult(res); Keyboard.dismiss();

      if (!overrideConfig && onSave) {
          const uChar = periodUnit === 'Years' ? 'y' : 'm';
          onSave(`RD: ${P} @ ${rate}% for ${period}${uChar}`, `Total: ${res.total}, Int: ${res.interest}`, "finance", "RD");
      }
  };
  ...
  const calculateRegularInterest = (overrideConfig) => {
      ...
      if (!d1 || !d2 || isNaN(d1) || isNaN(d2)) { if (!overrideConfig) alert("Invalid Date"); return; }
      
      let T_years = 0; let diffDays = 0; let R_annual = rawRate; let breakdown = { years:0, months:0, days:0 };
      if (calcMode30) {
          if (rateUnit === "Monthly") R_annual = rawRate * 12; else if (rateUnit === "Weekly") R_annual = rawRate * 52; else if (rateUnit === "Daily") R_annual = rawRate * 360;
          let yearDiff = d2.getFullYear() - d1.getFullYear(); let monthDiff = d2.getMonth() - d1.getMonth(); let dayDiff = d2.getDate() - d1.getDate();
          if (dayDiff < 0) { monthDiff -= 1; dayDiff += 30; } if (monthDiff < 0) { yearDiff -= 1; monthDiff += 12; }
          diffDays = (yearDiff * 360) + (monthDiff * 30) + dayDiff; T_years = diffDays / 360.0; breakdown = { years: yearDiff, months: monthDiff, days: dayDiff };
      } else {
          if (rateUnit === "Monthly") R_annual = rawRate * 12; else if (rateUnit === "Weekly") R_annual = rawRate * 52; else if (rateUnit === "Daily") R_annual = rawRate * 365;
          const diffTime = Math.abs(d2 - d1); diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); T_years = diffDays / 365.0; breakdown = getDurationBreakdown(d1, d2);
      }
      const displayMonthlyRate = parseFloat((R_annual / 12).toFixed(2));
      const getCompounded = (freqYears) => { if (freqYears <= 0) return P; const ratePerPeriod = (R_annual * freqYears) / 100; const numPeriods = T_years / freqYears; return P * Math.pow(1 + ratePerPeriod, numPeriods); };
      ...
      if (nthVal) {
          ...
          if (freqInYears > 0 && T_years >= freqInYears) {
              nthTotal = getCompounded(freqInYears); nthLabel = `${val} ${nthUnit}`;
          }
      }
      const format = (amt) => ({ interest: (amt - P).toFixed(2), total: amt.toFixed(2) });
      setRegResult({ diffDays, breakdown, startStr: toFriendlyDate(d1), endStr: toFriendlyDate(d2), isEndToday: isToday(d2), monthlyRate: displayMonthlyRate, simple: format(simpleTotal), yr3: yr3Total ? format(yr3Total) : null, yr2: yr2Total ? format(yr2Total) : null, yr1: yr1Total ? format(yr1Total) : null, nth: nthTotal ? format(nthTotal) : null, nthLabel, modeUsed: calcMode30 ? "30/360" : "Actual/365" });
      Keyboard.dismiss();
      if (!overrideConfig && onSave) { const modeTag = calcMode30 ? "30" : "Cal"; onSave(`Reg Int: ${P} @ ${rawRate}% ${rateUnit} | ${start} to ${end} | ${modeTag}`, `Days: ${diffDays}, Simp: ${format(simpleTotal).total}`, "finance", "Interest"); }
  };
  ...
  const calculateStockAvg = (overrideConfig) => {
    ...
    const qty1 = parseFloat(q1); const price1 = parseFloat(p1); const qty2 = parseFloat(q2); const price2 = parseFloat(p2);
    const oldAmt = qty1 * price1; const newAmt = qty2 * price2; const totalQty = qty1 + qty2; const totalAmt = oldAmt + newAmt; const avgPrice = totalAmt / totalQty;
    const res = { totalQty, totalAmt: totalAmt.toFixed(2), avg: avgPrice.toFixed(2), oldQty: qty1, oldPrice: price1, oldTotal: oldAmt.toFixed(2), newQty: qty2, newPrice: price2, newTotal: newAmt.toFixed(2) };
    setStockResult(res); Keyboard.dismiss();
    if (!overrideConfig && onSave) { onSave(`Stock Avg: ${q1}x${p1} + ${q2}x${p2}`, `Avg: ${res.avg}, Qty: ${totalQty}`, "finance", "Stocks"); }
  };
  ...
  const calculateInterestLegacy = (overrideConfig, overrideType) => {
      ...
      const P = parseFloat(principal); const R = parseFloat(rate); const T = parseFloat(time);
      let interest = 0; let total = 0;
      if (type === "Simple") { interest = (P * R * T) / 100; total = P + interest; } 
      else { let n = 1; const val = parseFloat(cmpVal) || 1; 
          if (cmpUnit === "Yearly") n = 1 / val; else if (cmpUnit === "Monthly") n = 12 / val; else if (cmpUnit === "Weekly") n = 52 / val; else if (cmpUnit === "Daily") n = 365 / val;
          const r = R / 100; total = P * Math.pow(1 + (r / n), n * T); interest = total - P; 
      }
      setIntResult({ interest: interest.toFixed(2), total: total.toFixed(2) }); Keyboard.dismiss();
      if (!overrideConfig && onSave) { onSave(`${type} Int: ${P} @ ${R}% for ${T}y`, `Int: ${interest.toFixed(2)}, Total: ${total.toFixed(2)}`, "finance", "Interest"); }
  }
  ...
  const calculateSIP = (overrideConfig) => {
      ...
      const P = parseFloat(investment); const n = parseFloat(years) * 12; const i = parseFloat(rate) / 100 / 12;
      const investedAmount = P * n; const totalValue = P * ((Math.pow(1 + i, n) - 1) / i) * (1 + i); const estReturns = totalValue - investedAmount;
      const res = { invested: investedAmount.toFixed(0), returns: estReturns.toFixed(0), total: totalValue.toFixed(0), returnPct: totalValue > 0 ? estReturns / totalValue : 0 };
      setSipResult(res); Keyboard.dismiss();
      if (!overrideConfig && onSave) { onSave(`SIP: ${P} @ ${rate}% for ${years}y`, `Total: ${res.total}, Ret: ${res.returns}`, "finance", "SIP"); }
  };
  ...
  const calculateGST = (overrideConfig, overrideInclusive) => {
      ...
      const basePrice = parseFloat(amount); const gstRate = parseFloat(rate);
      let gstAmount = 0; let netAmount = 0; let totalAmount = 0;
      if (inclusive) { netAmount = basePrice / (1 + (gstRate / 100)); gstAmount = basePrice - netAmount; totalAmount = basePrice; } 
      else { gstAmount = (basePrice * gstRate) / 100; netAmount = basePrice; totalAmount = basePrice + gstAmount; }
      setGstResult({ net: netAmount.toFixed(2), gst: gstAmount.toFixed(2), total: totalAmount.toFixed(2) }); Keyboard.dismiss();
      if (!overrideConfig && onSave) { const typeStr = inclusive ? "Inclusive" : "Exclusive"; onSave(`GST: ${amount} @ ${rate}% (${typeStr})`, `Total: ${totalAmount.toFixed(2)}, GST: ${gstAmount.toFixed(2)}`, "finance", "GST"); }
  };
        ...
        <View style={s.catWrap}>
            ...
            {CATEGORIES.map((cat) => (
                <TouchableOpacity key={cat} style={[s.catBtn, category === cat && s.catBtnActive]} 
                    onPress={() => { 
                        setCategory(cat); 
                        setEmiResult(null); setStockResult(null); setIntResult(null); setRegResult(null); setSipResult(null); setGstResult(null); setFdResult(null); setRdResult(null);
                    }}>
        ...
        <ScrollView contentContainerStyle={s.section}>
                    ...
                    {emiResult && (
                        <View style={s.resBox}>
                            ...
                            <View style={{ alignItems: 'center', marginBottom: 20 }}>
                                <PieChart2Segment radius={80} percent={emiResult.interestPct} colorFilled="#EF4444" colorEmpty="#4F46E5" totalValue={emiResult.total} formatFn={formatNumber} />
                                <View style={{ flexDirection: 'row', gap: 15, marginTop: 15 }}>
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}><View style={{width: 12, height: 12, borderRadius: 6, backgroundColor: '#4F46E5', marginRight: 6}}/><Text style={{color: colors.secTxt, fontSize: 12}}>Principal</Text></View>
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}><View style={{width: 12, height: 12, borderRadius: 6, backgroundColor: '#EF4444', marginRight: 6}}/><Text style={{color: colors.secTxt, fontSize: 12}}>Interest</Text></View>
                                </View>
                            </View>
            ...
            {category === "FD" && (
                <View>
                    ...
                    <TextInput style={s.input} value={fdConfig.amount} onChangeText={t => setFdConfig({...fdConfig, amount: t})} keyboardType="numeric" placeholder="e.g. 100000" placeholderTextColor={colors.secTxt} />
                    <View style={s.row}>
                        <View style={s.col}>
                            <Text style={s.label}>Interest Rate (%)</Text>
                            <TextInput style={s.input} value={fdConfig.rate} onChangeText={t => setFdConfig({...fdConfig, rate: t})} keyboardType="numeric" placeholder="e.g. 6.5" placeholderTextColor={colors.secTxt} />
                        </View>
                    </View>
                    <Text style={s.label}>Time Period</Text>
                    <View style={s.row}>
                        <View style={s.col}>
                            <TextInput style={s.input} value={fdConfig.period} onChangeText={t => setFdConfig({...fdConfig, period: t})} keyboardType="numeric" placeholder="e.g. 1" placeholderTextColor={colors.secTxt} />
                        </View>
                        <View style={s.col}>
                            <View style={s.toggleContainer}>
                                {['Years', 'Months', 'Days'].map(u => (
                                    <TouchableOpacity key={u} onPress={()=>setFdConfig({...fdConfig, periodUnit: u})} style={[s.toggleBtn, fdConfig.periodUnit===u && s.toggleBtnActive]}>
                                        <Text style={[s.toggleTxt, {color: fdConfig.periodUnit===u ? 'white' : colors.secTxt}]}>{u}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </View>
                    </View>
                    <Text style={s.label}>Compounding Frequency</Text>
                     <View style={s.toggleContainer}>
                        {['Quarterly', 'Monthly', 'Yearly'].map(u => (
                            <TouchableOpacity key={u} onPress={()=>setFdConfig({...fdConfig, freq: u})} style={[s.toggleBtn, fdConfig.freq===u && s.toggleBtnActive]}>
                                <Text style={[s.toggleTxt, {color: fdConfig.freq===u ? 'white' : colors.secTxt}]}>{u}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>

                    <TouchableOpacity style={s.calcBtn} onPress={() => calculateFD()}>
                        <Text style={s.calcBtnTxt}>Calculate</Text>
                    </TouchableOpacity>

                    {fdResult && (
                        <View style={s.resBox}>
                            <Text style={[s.resLabel, {textAlign:'center'}]}>Maturity Value</Text>
                            <Text style={[s.resBig, {textAlign:'center', marginBottom: 20}]}>{formatNumber(fdResult.total, 2)}</Text>
                            <View style={{ alignItems: 'center', marginBottom: 20 }}>
                                <PieChart2Segment radius={80} percent={fdResult.interestPct} colorFilled="#10B981" colorEmpty="#4F46E5" totalValue={fdResult.total} formatFn={formatNumber} />
                                <View style={{ flexDirection: 'row', gap: 15, marginTop: 15 }}>
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}><View style={{width: 12, height: 12, borderRadius: 6, backgroundColor: '#4F46E5', marginRight: 6}}/><Text style={{color: colors.secTxt, fontSize: 12}}>Invested</Text></View>
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}><View style={{width: 12, height: 12, borderRadius: 6, backgroundColor: '#10B981', marginRight: 6}}/><Text style={{color: colors.secTxt, fontSize: 12}}>Returns</Text></View>
                                </View>
                            </View>
                            <View style={{ borderTopWidth: 1, borderTopColor: colors.subTab, paddingTop: 15 }}>
                                <ResultRow label="Invested Amount:" value={fdResult.principal} color="#4F46E5" />
                                <ResultRow label="Est. Returns:" value={fdResult.interest} color="#10B981" />
                                <ResultRow label="Total Maturity:" value={fdResult.total} />
                            </View>
                        </View>
                    )}
                </View>
            ...
            {category === "RD" && (
                <View>
                    ...
                    <TextInput style={s.input} value={rdConfig.amount} onChangeText={t => setRdConfig({...rdConfig, amount: t})} keyboardType="numeric" placeholder="e.g. 5000" placeholderTextColor={colors.secTxt} />
                    <View style={s.row}>
                        <View style={s.col}>
                            <Text style={s.label}>Interest Rate (%)</Text>
                            <TextInput style={s.input} value={rdConfig.rate} onChangeText={t => setRdConfig({...rdConfig, rate: t})} keyboardType="numeric" placeholder="e.g. 6.5" placeholderTextColor={colors.secTxt} />
                        </View>
                    </View>
                    <Text style={s.label}>Time Period</Text>
                    <View style={s.row}>
                        <View style={s.col}>
                            <TextInput style={s.input} value={rdConfig.period} onChangeText={t => setRdConfig({...rdConfig, period: t})} keyboardType="numeric" placeholder="e.g. 1" placeholderTextColor={colors.secTxt} />
                        </View>
                        <View style={s.col}>
                            <View style={s.toggleContainer}>
                                {['Years', 'Months'].map(u => (
                                    <TouchableOpacity key={u} onPress={()=>setRdConfig({...rdConfig, periodUnit: u})} style={[s.toggleBtn, rdConfig.periodUnit===u && s.toggleBtnActive]}>
                                        <Text style={[s.toggleTxt, {color: rdConfig.periodUnit===u ? 'white' : colors.secTxt}]}>{u}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </View>
                     ...
                     <View style={s.toggleContainer}>
                        {['Quarterly', 'Monthly', 'Yearly'].map(u => (
                            <TouchableOpacity key={u} onPress={()=>setRdConfig({...rdConfig, freq: u})} style={[s.toggleBtn, rdConfig.freq===u && s.toggleBtnActive]}>
                                <Text style={[s.toggleTxt, {color: rdConfig.freq===u ? 'white' : colors.secTxt}]}>{u}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
                    
                    <TouchableOpacity style={s.calcBtn} onPress={() => calculateRD()}>
                        <Text style={s.calcBtnTxt}>Calculate</Text>
                    </TouchableOpacity>
                    ...
                    {rdResult && (
                        <View style={s.resBox}>
                            <Text style={[s.resLabel, {textAlign:'center'}]}>Maturity Value</Text>
                            <Text style={[s.resBig, {textAlign:'center', marginBottom: 20}]}>{formatNumber(rdResult.total, 2)}</Text>
                            <View style={{ alignItems: 'center', marginBottom: 20 }}>
                                <PieChart2Segment radius={80} percent={rdResult.interestPct} colorFilled="#10B981" colorEmpty="#4F46E5" totalValue={rdResult.total} formatFn={formatNumber} />
                                <View style={{ flexDirection: 'row', gap: 15, marginTop: 15 }}>
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}><View style={{width: 12, height: 12, borderRadius: 6, backgroundColor: '#4F46E5', marginRight: 6}}/><Text style={{color: colors.secTxt, fontSize: 12}}>Invested</Text></View>
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}><View style={{width: 12, height: 12, borderRadius: 6, backgroundColor: '#10B981', marginRight: 6}}/><Text style={{color: colors.secTxt, fontSize: 12}}>Returns</Text></View>
                                </View>
                            ...
                            <View style={{ borderTopWidth: 1, borderTopColor: colors.subTab, paddingTop: 15 }}>
                                <ResultRow label="Total Invested:" value={rdResult.invested} color="#4F46E5" />
                                <ResultRow label="Est. Returns:" value={rdResult.interest} color="#10B981" />
                                <ResultRow label="Total Maturity:" value={rdResult.total} />
                            </View>
                        </View>
                    )}
                </View>
                        ...
                        <View>
                             ...
                             <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between', marginTop:15, marginBottom:5}}>
                                ...
                                <View style={{flexDirection:'row', backgroundColor: colors.btn, borderRadius:20, padding:2, borderWidth:1, borderColor:'#333'}}>
                                    <TouchableOpacity onPress={()=>setIs30DayMode(true)} style={{paddingHorizontal:12, paddingVertical:6, borderRadius:18, backgroundColor: is30DayMode ? colors.accent : 'transparent'}}><Text style={{fontSize:12, fontWeight:'bold', color: is30DayMode ? '#fff' : colors.secTxt}}>30 Days/M</Text></TouchableOpacity>
                                    <TouchableOpacity onPress={()=>setIs30DayMode(false)} style={{paddingHorizontal:12, paddingVertical:6, borderRadius:18, backgroundColor: !is30DayMode ? colors.accent : 'transparent'}}><Text style={{fontSize:12, fontWeight:'bold', color: !is30DayMode ? '#fff' : colors.secTxt}}>Calendar days</Text></TouchableOpacity>
                                </View>
                             </View>
                                 ...
                                 <View style={s.col}>
                                      <View style={s.toggleContainer}>
                                          {['Y', 'M', 'W', 'D'].map(u => {
                                               ...
                                               return (
                                              <TouchableOpacity key={u} onPress={()=>setRegConfig({...regConfig, nthUnit: full})} style={[s.toggleBtn, regConfig.nthUnit===full && s.toggleBtnActive]}>
                            ...
                            {regResult && (
                                <View style={s.resBox}>
                                    <Text style={s.headerLine}>Monthly Interest Rate: <Text style={s.highlight}>{regResult.monthlyRate}%</Text></Text>
                                    <Text style={s.headerLine}>Calculation Mode: <Text style={s.highlight}>{regResult.modeUsed}</Text></Text>
                                    <Text style={s.headerLine}>From <Text style={s.highlight}>{regResult.startStr}</Text> to <Text style={s.highlight}>{regResult.endStr}</Text>{regResult.isEndToday ? " (today)" : ""}</Text>
                                    <Text style={[s.headerLine, s.highlight]}>Total Duration :{`  ${regResult.breakdown.years}y ${regResult.breakdown.months}m ${regResult.breakdown.days}d`}</Text>
                        ...
                        <View>
                            ...
                            {intType === "Compound" && (
                                <View>
                                    ...
                                    <View style={s.row}>
                                        <View style={{ flex: 0.8 }}>
                                            <TextInput style={s.input} value={legacyConfig.cmpVal} onChangeText={t => setLegacyConfig({...legacyConfig, cmpVal: t})} keyboardType="numeric" placeholder="1" placeholderTextColor={colors.secTxt}/>
                                        </View>
                                        <View style={{ flex: 2.2 }}>
                                            <View style={s.toggleContainer}>
                                                {['Yearly', 'Monthly', 'Weekly', 'Daily'].map(u => (
                                                    <TouchableOpacity key={u} onPress={()=>setLegacyConfig({...legacyConfig, cmpUnit: u})} style={[s.toggleBtn, legacyConfig.cmpUnit===u && s.toggleBtnActive, {paddingHorizontal: 2}]}>
                ...
                <View>
                    ...
                    {sipResult && (
                        <View style={s.resBox}>
                            ...
                            <View style={{ alignItems: 'center', marginBottom: 20 }}>
                                <PieChart2Segment radius={80} percent={sipResult.returnPct} colorFilled="#10B981" colorEmpty="#4F46E5" totalValue={sipResult.total} formatFn={formatNumber} />
                                <View style={{ flexDirection: 'row', gap: 15, marginTop: 15 }}>
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}><View style={{width: 12, height: 12, borderRadius: 6, backgroundColor: '#4F46E5', marginRight: 6}}/><Text style={{color: colors.secTxt, fontSize: 12}}>Invested</Text></View>
                                    <View style={{flexDirection: 'row', alignItems: 'center'}}><View style={{width: 12, height: 12, borderRadius: 6, backgroundColor: '#10B981', marginRight: 6}}/><Text style={{color: colors.secTxt, fontSize: 12}}>Returns</Text></View>
                                </View>
            ...
            {category === "GST" && (
                <View>
                    ...
                    <View style={{flexDirection:'row', flexWrap:'wrap', gap: 10, marginBottom: 15}}>
                        {["5", "12", "18", "28"].map((r) => (
                             <TouchableOpacity key={r} onPress={() => setGstConfig({...gstConfig, rate: r})} style={{backgroundColor: gstConfig.rate === r ? colors.accent : colors.subTab, paddingHorizontal: 15, paddingVertical: 8, borderRadius: 20}}>
                    ...
                    <View style={{marginTop: 20, marginBottom: 10}}>
                         <View style={{flexDirection:'row', backgroundColor: colors.btn, borderRadius:10, borderWidth:1, borderColor:'#333', overflow:'hidden'}}>
                            <TouchableOpacity onPress={()=>setIsGstInclusive(false)} style={[s.toggleBtn, !isGstInclusive && s.toggleBtnActive]}><Text style={[s.toggleTxt, {color: !isGstInclusive ? 'white' : colors.secTxt}]}>Exclusive (Add GST)</Text></TouchableOpacity>
                            <TouchableOpacity onPress={()=>setIsGstInclusive(true)} style={[s.toggleBtn, isGstInclusive && s.toggleBtnActive]}><Text style={[s.toggleTxt, {color: isGstInclusive ? 'white' : colors.secTxt}]}>Inclusive (Remove GST)</Text></TouchableOpacity>
                        </View>
                    </View>
