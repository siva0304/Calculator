...
import { StyleSheet, Text, View, TextInput, TouchableOpacity, ScrollView, Dimensions, Keyboard } from "react-native";
...
export default function Finance(props) {
  const { 
    onSave, colors, showHistory, globalHistory, onClear, onRemoveItem, 
    onRequestNewTab, pendingLoad, onConsumeLoad, formatNumber // Received from App.js
  } = props;
  ...
  useEffect(() => {
    if (pendingLoad && pendingLoad.targetTabId === props.tabId) {
      const isFinance = pendingLoad.expression.match(/^(EMI:|Stock Avg:|Reg Int:|Simple Int:|Compound Int:|SIP:|GST:)/);
      if (!isFinance) return;
      loadFromExpression(pendingLoad.expression);
      if (onConsumeLoad) onConsumeLoad();
    }
  }, [pendingLoad]);
  ...
  const calculateEMI = (overrideConfig) => {
    ...
    if (!overrideConfig && onSave) {
        ...
        onSave(`EMI: ${P} @ ${rate}% for ${tenure}${unitChar}`, `EMI: ${res.emi}, Total: ${res.total}`);
    }
  };
  ...
  const calculateRegularInterest = (overrideConfig) => {
      ...
      if (!overrideConfig && onSave) {
          ...
          onSave(`Reg Int: ${P} @ ${rawRate}% ${rateUnit} | ${start} to ${end} | ${modeTag}`, `Days: ${diffDays}, Simp: ${format(simpleTotal).total}`);
      }
  };
  ...
  const calculateStockAvg = (overrideConfig) => {
    ...
    Keyboard.dismiss();
    if (!overrideConfig && onSave) {
        onSave(`Stock Avg: ${q1}x${p1} + ${q2}x${p2}`, `Avg: ${res.avg}, Qty: ${totalQty}`);
    }
  };
  ...
  const calculateInterestLegacy = (overrideConfig, overrideType) => {
      ...
      Keyboard.dismiss();
      if (!overrideConfig && onSave) {
          onSave(`${type} Int: ${P} @ ${R}% for ${T}y`, `Int: ${interest.toFixed(2)}, Total: ${total.toFixed(2)}`);
      }
  }
  ...
  const calculateSIP = (overrideConfig) => {
      ...
      if (!overrideConfig && onSave) {
          onSave(`SIP: ${P} @ ${rate}% for ${years}y`, `Total: ${res.total}, Ret: ${res.returns}`);
      }
  };
  ...
  const calculateGST = (overrideConfig, overrideInclusive) => {
      ...
      if (!overrideConfig && onSave) {
          ...
          onSave(`GST: ${amount} @ ${rate}% (${typeStr})`, `Total: ${totalAmount.toFixed(2)}, GST: ${gstAmount.toFixed(2)}`);
      }
  };

  const isFormEmpty = () => {
    if (category === "EMI") return !emiConfig.amount && !emiConfig.rate;
    if (category === "Stocks") return !stockConfig.q1 && !stockConfig.q2;
    if (category === "Interest") return intType === "Regular" ? !regConfig.principal : !legacyConfig.principal;
    if (category === "SIP") return !sipConfig.investment;
    if (category === "GST") return !gstConfig.amount;
    return true;
  };

  const handleHistorySelect = (expr) => {
    const isFinanceExpr = expr.match(/^(EMI:|Stock Avg:|Reg Int:|Simple Int:|Compound Int:|SIP:|GST:)/);
    const empty = isFormEmpty();

    if (empty && isFinanceExpr) {
      loadFromExpression(expr);
      if (props.setShowHistory) props.setShowHistory(false);
    } else {
      if (onRequestNewTab) onRequestNewTab(expr, empty);
    }
  };

  const s = StyleSheet.create({
    container: { flex: 1, backgroundColor: colors.bg },
    // MODIFIED: Approx 2 rows visible + Scroll
    ...
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg, padding: 15 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
            <Text style={{ color: colors.secTxt, fontWeight: 'bold' }}>FINANCE HISTORY</Text>
           ...
           <ScrollView>
            {globalHistory.slice().reverse().map((item) => (
                <View key={item.id} style={{ backgroundColor: colors.btn, borderRadius: 15, padding: 15, marginBottom: 10 }}>
                    <TouchableOpacity onPress={() => handleHistorySelect(item.expression)}>
                        <Text style={{ color: colors.txt, fontSize: 16 }}>{item.expression}</Text>
                        <Text style={{ color: colors.accent, fontSize: 16, fontWeight: "bold", textAlign: "right" }}>= {formatNumber(item.result, 2)}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => onRemoveItem && onRemoveItem(item.id)} style={{ position: 'absolute', top: 5, right: 10 }}>
                        <Text style={{ color: colors.secTxt, fontSize: 18 }}>×</Text>
                    </TouchableOpacity>
                </View>
            ))}
           </ScrollView>
  ...
  return (
    <View style={s.container}>
        {/* Categories Grid - Multi Row with Scroll */}
